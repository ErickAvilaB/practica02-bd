...
class SucursalService:
    """
    Clase que implementa los casos de uso (CRUD) para la entidad Sucursal.
    ...
    Trabaja con:
    - Un repositorio CSV para persistencia
    - Un validador para entrada de datos
    """
    ...
    def __init__(self, repository: CsvRepository, validator: InputValidator) -> None:
        ...
        :param repository: Repositorio que maneja la persistencia en CSV.
        :param validator: Componente encargado de validar entradas del usuario.
        """
    ...
    def list_brief(self) -> None:
        """
        Lista todas las sucursales mostrando únicamente id y nombre.
        """
    ...
    def consult_by_id(self) -> None:
        """
        Solicita un id al usuario y muestra todos los datos
        de la sucursal correspondiente.
        """
        key_value = self._validator.require_existing_id(
            input("Ingresa el id de la sucursal: ")
        )
    ...
    def create_interactive(self) -> None:
        """
        Solicita los datos por consola para crear una nueva sucursal
        ...
        # Teléfono: se limpia y valida que contenga solo dígitos
        ...
        if not telefono_clean.isdigit():
            raise AppError(
                ...
                "(puedes usar espacios o guiones)."
            )

        horario_atencion = self._validator.read_text(
            "Horario de atencion (ej. L-V 09:00-18:00): "
        )

        # Datos de clínica (opcionales)
        tiene_clinica = self._validator.read_bool(
            "Tiene clinica integrada? (s/n): ",
            default=False
        )
        ...
        if tiene_clinica:
            ...
            clinica_numero_cuartos = self._validator.read_int(
                "Numero de cuartos (entero): "
            )
            clinica_numero_empleados = self._validator.read_int(
                "Numero de empleados (entero): "
            )
    ...
    def edit_interactive(self) -> None:
        """
        Permite editar una sucursal existente.
        Si el usuario presiona Enter, se conserva el valor actual.
        """
        key_value = self._validator.require_existing_id(
            input("Ingresa el id de la sucursal a editar: ")
        )
        ...
        # Se muestran valores actuales como referencia
        ...
        telefono = self._validator.read_text(
            f"Telefono [{current.telefono}]: ",
            allow_empty=True,
        ...
        # Se conservan valores actuales por defecto
        ...
        if tiene_clinica:
            ...
            # Si se desactiva la clínica, se limpian los datos asociados
            nombre_clinica = ""
            clinica_numero_cuartos = None
            clinica_numero_empleados = None
            clinica_horario_atencion = ""
    ...
    def delete_interactive(self) -> None:
        """
        Elimina una sucursal solicitando confirmación al usuario.
        """
        key_value = self._validator.require_existing_id(
            input("Ingresa el id de la sucursal a eliminar: ")
        )
    ...
    def _print_full(self, sucursal: Sucursal) -> None:
        """
        Imprime todos los campos de una sucursal de forma detallada.
        """
        print("\n--- Detalle de sucursal ---")
