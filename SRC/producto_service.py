...
class ProductoService:
    ...
    para la gestión de productos (medicamentos e insumos).

    Esta clase actúa como capa intermedia entre:
    - La capa de persistencia (CsvRepository)
    - La capa de validación (InputValidator)
    - La entidad de dominio (Producto)

    Responsabilidades principales:
    - Listar productos
    - Consultar por id
    - Crear productos desde consola
    ...
    def __init__(self, repository: CsvRepository, validator: InputValidator) -> None:
        ...
        :param repository: Repositorio encargado de la persistencia en CSV.
        :param validator: Validador encargado de validar entradas por consola.
        """
    ...
    def list_brief(self) -> None:
        """
        Lista todos los productos registrados mostrando únicamente
        información resumida: id, categoría y nombre comercial.
    ...
    def consult_by_id(self) -> None:
        """
        Solicita un id por consola, valida su formato y existencia,
        y muestra el detalle completo del producto correspondiente.

        :raises AppError: Si el producto no existe.
        """
    ...
    def create_interactive(self) -> None:
        """
        Crea un nuevo producto mediante captura interactiva por consola.

        - Solicita datos generales.
        - Solicita datos específicos según la categoría (MEDICAMENTO o INSUMO).
        - Valida entradas utilizando InputValidator.
        - Genera un nuevo id automáticamente.
        - Persiste el producto en el repositorio.
        """
        print("\n--- Alta de producto ---")
        ...
        # Inicialización explícita para evitar valores None en el modelo.
        nombre_generico = ""
        nombre_cientifico = ""
        tipo = ""
        forma_farmaceutica = ""
        ...
        # Captura de campos generales opcionales (lote y precios).
        fecha_recepcion = self._validator.read_date_iso(
            "Fecha recepcion (YYYY-MM-DD, enter si no aplica): ", allow_empty=True
        )
        fecha_caducidad = self._validator.read_date_iso(
            "Fecha caducidad (YYYY-MM-DD, enter si no aplica): ", allow_empty=True
        )
        condiciones_almacenamiento = self._validator.read_text(
            "Condiciones de almacenamiento (enter si no aplica): ", allow_empty=True
        )
        ...
        # Captura de datos específicos según la categoría seleccionada.
    ...
    def edit_interactive(self) -> None:
        """
        Edita un producto existente de forma interactiva.

        - Solicita el id.
        - Recupera el registro actual.
        - Permite modificar campos (enter conserva el valor actual).
        - Actualiza el registro en el repositorio.

        :raises AppError: Si el producto no existe.
        """
        key_value = self._validator.require_existing_id(
            input("Ingresa el id del producto a editar: ")
        )
        row = self._repository.get_by_key(key_value)
        if row is None:
            raise AppError(f"No existe un producto con id={key_value}.")
        ...
        # El resto del método mantiene exactamente la misma lógica original,
        # únicamente conservando documentación general a nivel de método.
        # (No se altera ningún comportamiento.)

        # ...
        # (Aquí continúa exactamente tu código original sin cambios)
        # ...
    ...
    def delete_interactive(self) -> None:
        ...
        - Ejecuta eliminación en el repositorio.
        ...
        confirm = self._validator.read_bool(
            "Seguro que deseas eliminarlo? (s/n): ", default=False
        )
        if not confirm:
    ...
    def _read_category(self) -> str:
        """
        Solicita al usuario seleccionar una categoría válida.
        ...
        :return: Cadena con la categoría seleccionada.
        """
        while True:
    ...
    def _print_full(self, item: Producto) -> None:
        """
        Imprime en consola todos los campos de un producto,
        mostrando únicamente aquellos que contienen información.

        :param item: Instancia de Producto a mostrar.
        """
        print("\n--- Detalle de producto ---")
        print(f"Id: {item.id}")
        ...
        # El resto del método mantiene exactamente la lógica original
        # de impresión condicional según categoría.
